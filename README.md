...
| lsd.core.report.useLocalStaticFiles | true              | Whether to add and reference the lsd specific css and javascript files locally. (Rather than a remote http reference which will point to the latest version but may have bugs/issues if the library version is not up to date).                               |
